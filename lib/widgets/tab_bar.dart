...
class TmTabBar extends StatefulWidget {
  ...
  @override
  State<TmTabBar> createState() => _TmTabBarState();
}

class _TmTabBarState extends State<TmTabBar>
    with SingleTickerProviderStateMixin {
  static const double kBubbleR = 27.0;
  static const double kBarH = 62.0;
  static const double kOverhang = 18.0;
  static const double kHMargin = 16.0;

  late final AnimationController _ctrl;
  late Animation<double> _tabPos;
  ...
  @override
  void didUpdateWidget(covariant TmTabBar old) {
    super.didUpdateWidget(old);
    if (old.active != widget.active) {
      _tabPos = Tween<double>(
        begin: old.active.index.toDouble(),
        end: widget.active.index.toDouble(),
    ...
    return AnimatedBuilder(
      ...
      builder: (ctx, _) => LayoutBuilder(
        builder: (ctx, constraints) {
          ...
          final tabPos = _tabPos.value;
          final notchAbsX = kHMargin + (tabPos + 0.5) * tabW;
          final notchRelX = (tabPos + 0.5) * tabW;
          final bubbleLeft = notchAbsX - kBubbleR;
          final barTop = kBubbleR - kOverhang;
